...
def device_group_metrics_render(device_group_name, metric):
    ...
    url = "%s/render/?target=aliasByNode(cactiStyle(aliasByNode(%s.{%s}.%s,1)),3)&from=%s&height=400&width=800&title=%s&bgcolor=white&fgcolor=black&drawNullAsZero=false&lineMode=connected&colorList=green,blue,yellow,blue,black,orange,pink&yMin=0"%(
        config['metrics']['render_api_base_url'],
        config['metrics']['prefix'],
        device_list,
        metric,
